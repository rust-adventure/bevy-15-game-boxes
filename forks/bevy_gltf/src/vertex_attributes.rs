use bevy_render::{
    mesh::{
        MeshVertexAttribute,
        VertexAttributeValues as Values,
    },
    prelude::Mesh,
...
use gltf::{
    ...
    mesh::util::{
        ReadColors, ReadJoints, ReadTexCoords, ReadWeights,
    },
};
...
impl<'a> BufferAccessor<'a> {
    ...
    fn iter<T: gltf::accessor::Item>(
        self,
    ) -> Result<gltf::accessor::Iter<'a, T>, AccessFailed>
    {
        gltf::accessor::Iter::new(
            self.accessor,
            |buffer: gltf::Buffer| {
                self.buffer_data
                    .get(buffer.index())
...
enum VertexAttributeIter<'a> {
    ...
    S16x2(
        gltf::accessor::Iter<'a, [i16; 2]>,
        Normalization,
    ),
    U16x2(
        gltf::accessor::Iter<'a, [u16; 2]>,
        Normalization,
    ),
    S16x4(
        gltf::accessor::Iter<'a, [i16; 4]>,
        Normalization,
    ),
    U16x4(
        gltf::accessor::Iter<'a, [u16; 4]>,
        Normalization,
    ),
    S8x2(
        gltf::accessor::Iter<'a, [i8; 2]>,
        Normalization,
    ),
    U8x2(
        gltf::accessor::Iter<'a, [u8; 2]>,
        Normalization,
    ),
    S8x4(
        gltf::accessor::Iter<'a, [i8; 4]>,
        Normalization,
    ),
    U8x4(
        gltf::accessor::Iter<'a, [u8; 4]>,
        Normalization,
    ),
    // Additional on-disk formats used for RGB colors
    U16x3(
        gltf::accessor::Iter<'a, [u16; 3]>,
        Normalization,
    ),
    U8x3(
        gltf::accessor::Iter<'a, [u8; 3]>,
        Normalization,
    ),
}
...
impl<'a> VertexAttributeIter<'a> {
    ...
    fn from_accessor(
        ...
        let normalization =
            Normalization(accessor.normalized());
        let format = (
            accessor.data_type(),
            accessor.dimensions(),
        );
        ...
        match format {
            (DataType::F32, Dimensions::Scalar) => {
                acc.with_no_norm(VertexAttributeIter::F32)
            }
            (DataType::U32, Dimensions::Scalar) => {
                acc.with_no_norm(VertexAttributeIter::U32)
            }
            (DataType::F32, Dimensions::Vec2) => {
                acc.with_no_norm(VertexAttributeIter::F32x2)
            }
            (DataType::U32, Dimensions::Vec2) => {
                acc.with_no_norm(VertexAttributeIter::U32x2)
            }
            (DataType::F32, Dimensions::Vec3) => {
                acc.with_no_norm(VertexAttributeIter::F32x3)
            }
            (DataType::U32, Dimensions::Vec3) => {
                acc.with_no_norm(VertexAttributeIter::U32x3)
            }
            (DataType::F32, Dimensions::Vec4) => {
                acc.with_no_norm(VertexAttributeIter::F32x4)
            }
            (DataType::U32, Dimensions::Vec4) => {
                acc.with_no_norm(VertexAttributeIter::U32x4)
            }
            (DataType::I16, Dimensions::Vec2) => {
                acc.with_norm(VertexAttributeIter::S16x2)
            }
            (DataType::U16, Dimensions::Vec2) => {
                acc.with_norm(VertexAttributeIter::U16x2)
            }
            (DataType::I16, Dimensions::Vec4) => {
                acc.with_norm(VertexAttributeIter::S16x4)
            }
            (DataType::U16, Dimensions::Vec4) => {
                acc.with_norm(VertexAttributeIter::U16x4)
            }
            (DataType::I8, Dimensions::Vec2) => {
                acc.with_norm(VertexAttributeIter::S8x2)
            }
            (DataType::U8, Dimensions::Vec2) => {
                acc.with_norm(VertexAttributeIter::U8x2)
            }
            (DataType::I8, Dimensions::Vec4) => {
                acc.with_norm(VertexAttributeIter::S8x4)
            }
            (DataType::U8, Dimensions::Vec4) => {
                acc.with_norm(VertexAttributeIter::U8x4)
            }
            (DataType::U16, Dimensions::Vec3) => {
                acc.with_norm(VertexAttributeIter::U16x3)
            }
            (DataType::U8, Dimensions::Vec3) => {
                acc.with_norm(VertexAttributeIter::U8x3)
            }
    ...
    fn into_any_values(
        ...
        match self {
            VertexAttributeIter::F32(it) => {
                Ok(Values::Float32(it.collect()))
            }
            VertexAttributeIter::U32(it) => {
                Ok(Values::Uint32(it.collect()))
            }
            VertexAttributeIter::F32x2(it) => {
                Ok(Values::Float32x2(it.collect()))
            }
            VertexAttributeIter::U32x2(it) => {
                Ok(Values::Uint32x2(it.collect()))
            }
            VertexAttributeIter::F32x3(it) => {
                Ok(Values::Float32x3(it.collect()))
            }
            VertexAttributeIter::U32x3(it) => {
                Ok(Values::Uint32x3(it.collect()))
            }
            VertexAttributeIter::F32x4(it) => {
                Ok(Values::Float32x4(it.collect()))
            }
            VertexAttributeIter::U32x4(it) => {
                Ok(Values::Uint32x4(it.collect()))
            }
            VertexAttributeIter::S16x2(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Snorm16x2,
                    Values::Sint16x2,
                )),
            VertexAttributeIter::U16x2(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Unorm16x2,
                    Values::Uint16x2,
                )),
            VertexAttributeIter::S16x4(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Snorm16x4,
                    Values::Sint16x4,
                )),
            VertexAttributeIter::U16x4(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Unorm16x4,
                    Values::Uint16x4,
                )),
            VertexAttributeIter::S8x2(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Snorm8x2,
                    Values::Sint8x2,
                )),
            VertexAttributeIter::U8x2(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Unorm8x2,
                    Values::Uint8x2,
                )),
            VertexAttributeIter::S8x4(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Snorm8x4,
                    Values::Sint8x4,
                )),
            VertexAttributeIter::U8x4(it, n) => Ok(n
                .apply_either(
                    it.collect(),
                    Values::Unorm8x4,
                    Values::Uint8x4,
    ...
    fn into_joint_index_values(
        ...
        match self {
            VertexAttributeIter::U8x4(
                ...
                ReadJoints::U8(it).into_u16().collect(),
            )),
            s => s.into_any_values(),
        }
    }
    ...
    fn into_joint_weight_values(
        ...
        match self {
            VertexAttributeIter::U8x4(
                ...
                ReadWeights::U8(it).into_f32().collect(),
            )),
            VertexAttributeIter::U16x4(
                it,
                Normalization(true),
            ) => Ok(Values::Float32x4(
                ReadWeights::U16(it).into_f32().collect(),
            )),
            s => s.into_any_values(),
        }
    }
    ...
    fn into_tex_coord_values(
        self,
    ) -> Result<Values, AccessFailed> {
        match self {
            VertexAttributeIter::U8x2(
                it,
                Normalization(true),
            ) => Ok(Values::Float32x2(
                ReadTexCoords::U8(it).into_f32().collect(),
            )),
            VertexAttributeIter::U16x2(
                it,
                Normalization(true),
            ) => Ok(Values::Float32x2(
...
pub(crate) fn convert_attribute(
    ...
    custom_vertex_attributes: &HashMap<
        Box<str>,
        MeshVertexAttribute,
    >,
) -> Result<
    (MeshVertexAttribute, Values),
    ConvertAttributeError,
> {
    if let Some((attribute, conversion)) = match &semantic {
        ...
        gltf::Semantic::Colors(0) => Some((
            Mesh::ATTRIBUTE_COLOR,
            ConversionMode::Rgba,
        )),
        gltf::Semantic::TexCoords(0) => Some((
            Mesh::ATTRIBUTE_UV_0,
            ConversionMode::TexCoord,
        )),
        gltf::Semantic::TexCoords(1) => Some((
            Mesh::ATTRIBUTE_UV_1,
            ConversionMode::TexCoord,
        )),
        ...
        let raw_iter = VertexAttributeIter::from_accessor(
            accessor.clone(),
            buffer_data,
        );
        let converted_values =
            raw_iter.and_then(|iter| match conversion {
                ConversionMode::Any => {
                    iter.into_any_values()
                }
                ConversionMode::Rgba => {
                    iter.into_rgba_values()
                }
                ConversionMode::TexCoord => {
                    iter.into_tex_coord_values()
                }
                ConversionMode::JointIndex => {
                    iter.into_joint_index_values()
                }
                ConversionMode::JointWeight => {
        ...
        match converted_values {
            ...
            Err(err) => {
                Err(ConvertAttributeError::AccessFailed(
                    err,
                    accessor.index(),
                ))
            }
        ...
        Err(ConvertAttributeError::UnknownName(
            semantic.to_string(),
        ))
    }
}
