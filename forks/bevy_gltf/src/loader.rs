use crate::{
    vertex_attributes::convert_attribute, Gltf,
    GltfAssetLabel, GltfExtras, GltfMaterialExtras,
    GltfMaterialName, GltfMeshExtras, GltfNode,
    GltfSceneExtras, GltfSkin,
};

use alloc::collections::VecDeque;
use bevy_asset::{
    io::Reader, AssetLoadError, AssetLoader, Handle,
    LoadContext, ReadAssetBytesError,
};
...
use bevy_hierarchy::{
    BuildChildren, ChildBuild, WorldChildBuilder,
};
use bevy_image::{
    CompressedImageFormats, Image, ImageAddressMode,
    ImageFilterMode, ImageLoaderSettings, ImageSampler,
    ImageSamplerDescriptor, ImageType, TextureError,
};
use bevy_math::{Affine2, Mat4, Vec3};
use bevy_pbr::{
    DirectionalLight, MeshMaterial3d, PointLight,
    SpotLight, StandardMaterial, UvChannel, MAX_JOINTS,
};
use bevy_render::{
    alpha::AlphaMode,
    camera::{
    ...
    mesh::{
        morph::{
            MeshMorphWeights, MorphAttributes,
            MorphTargetImage, MorphWeights,
        },
        skinning::{
            SkinnedMesh, SkinnedMeshInverseBindposes,
        },
        Indices, Mesh, Mesh3d, MeshVertexAttribute,
...
use gltf::{
    ...
    texture::{
        Info, MagFilter, MinFilter, TextureTransform,
        WrappingMode,
    },
...
use {
    bevy_animation::{
        prelude::*, AnimationTarget, AnimationTargetId,
    },
    smallvec::SmallVec,
};
...
pub enum GltfError {
    ...
    #[display(
        "failed to read bytes from an asset path: {_0}"
    )]
    ReadAssetBytesError(ReadAssetBytesError),
    /// Failed to load asset from an asset path.
    #[display(
        "failed to load asset from an asset path: {_0}"
    )]
    ...
    GenerateTangentsError(
        bevy_render::mesh::GenerateTangentsError,
    ),
...
pub struct GltfLoader {
    ...
    pub supported_compressed_formats:
    ...
    pub custom_vertex_attributes:
        HashMap<Box<str>, MeshVertexAttribute>,
}
...
impl AssetLoader for GltfLoader {
    ...
    async fn load(
        ...
        load_gltf(self, &bytes, load_context, settings)
            .await
    }
...
async fn load_gltf<'a, 'b, 'c>(
    ...
    let file_name = load_context
        ...
        .path()
        .to_str()
        .ok_or(GltfError::Gltf(gltf::Error::Io(
            Error::new(
        ...
        .to_string();
    let buffer_data =
        load_buffers(&gltf, load_context).await?;
    ...
    let paths = {
        let mut paths =
            HashMap::<usize, (usize, Vec<Name>)>::new();
    ...
    let (animations, named_animations, animation_roots) = {
        use bevy_animation::{
            animated_field, animation_curves::*,
            gltf_curves::*, VariableCurve,
        };
        use bevy_math::curve::{
            ConstantCurve, Interval, UnevenSampleAutoCurve,
        };
        ...
        for animation in gltf.animations() {
            ...
            for channel in animation.channels() {
                ...
                let keyframe_timestamps: Vec<f32> =
                    if let Some(inputs) =
                        reader.read_inputs()
                    {
                        match inputs {
                ...
                let maybe_curve: Option<VariableCurve> =
                    if let Some(outputs) =
                        reader.read_outputs()
                    {
                        match outputs {
                ...
                if let Some((root_index, path)) =
                    paths.get(&node.index())
                {
                    animation_roots.insert(*root_index);
                    animation_clip
                        .add_variable_curve_to_target(
        ...
        || cfg!(target_arch = "wasm32")
    {
        for texture in gltf.textures() {
            ...
            .await?;
            process_loaded_texture(
                load_context,
                &mut _texture_handles,
                image,
            );
        }
    } else {
    ...
    let mut meshes_on_non_skinned_nodes =
        HashSet::default();
    ...
    for gltf_mesh in gltf.meshes() {
        ...
        for primitive in gltf_mesh.primitives() {
            ...
            let primitive_topology =
                get_primitive_topology(primitive.mode())?;
                ...
                primitive.attributes()
            {
                ...
                .contains(&semantic)
                {
                    if !meshes_on_skinned_nodes
                        .contains(&gltf_mesh.index())
                    {
                        warn!(
                        ...
                        continue;
                    } else if meshes_on_non_skinned_nodes
                        .contains(&gltf_mesh.index())
                    {
            ...
            let reader = primitive.reader(|buffer| {
                Some(buffer_data[buffer.index()].as_slice())
            });
            if let Some(indices) = reader.read_indices() {
                mesh.insert_indices(match indices {
                    ReadIndices::U8(is) => Indices::U16(
                        is.map(|x| x as u16).collect(),
                    ),
                    ReadIndices::U16(is) => {
                        Indices::U16(is.collect())
                    }
                    ReadIndices::U32(is) => {
                        Indices::U32(is.collect())
                    }
                });
            };

            {
                let morph_target_reader =
                ...
                )
            {
                ...
                let generate_tangents_span = info_span!(
                    "generate_tangents",
                    name = file_name
                );
            ...
            primitives.push(super::GltfPrimitive::new(
                ...
                &primitive,
                mesh_handle,
                primitive.material().index().and_then(
                    |i| materials.get(i).cloned(),
                ),
                get_gltf_extras(primitive.extras()),
                get_gltf_extras(
                    primitive.material().extras(),
                ),
            ));
        }
        ...
        let mesh = super::GltfMesh::new(
            &gltf_mesh,
            primitives,
            get_gltf_extras(gltf_mesh.extras()),
        );

        let handle = load_context.add_labeled_asset(
            mesh.asset_label().to_string(),
            mesh,
        );
    ...
    let skinned_mesh_inverse_bindposes: Vec<_> = gltf
        .skins()
        .map(|gltf_skin| {
            let reader = gltf_skin.reader(|buffer| {
                Some(&buffer_data[buffer.index()])
            });
            let local_to_bone_bind_matrices: Vec<Mat4> =
                reader
    ...
    let mut nodes =
        HashMap::<usize, Handle<GltfNode>>::new();
    ...
    for node in GltfTreeIterator::try_new(&gltf)? {
        ...
        let children = node
            .children()
            .map(|child| {
                nodes.get(&child.index()).unwrap().clone()
            })
            .collect();
        ...
        #[cfg(feature = "bevy_animation")]
        let gltf_node = gltf_node.with_animation_root(
            animation_roots.contains(&node.index()),
        );

        let handle = load_context.add_labeled_asset(
            gltf_node.asset_label().to_string(),
            gltf_node,
        );
    ...
    for scene in gltf.scenes() {
        ...
        for (&entity, &skin_index) in
            &entity_to_skin_index_map
        {
            let mut entity = world.entity_mut(entity);
            let skin =
                gltf.skins().nth(skin_index).unwrap();
            let joint_entities: Vec<_> = skin
                .joints()
                .map(|node| {
                    node_index_to_entity_map[&node.index()]
                })
                .collect();
        ...
        let loaded_scene = scene_load_context
            .finish(Scene::new(world), None);
        ...
        if let Some(name) = scene.name() {
            named_scenes
                .insert(name.into(), scene_handle.clone());
        }
...
fn paths_recur(
    ...
    for child in node.children() {
        if !visited.contains(&child.index()) {
            paths_recur(
                child, &path, paths, root_index, visited,
            );
        }
    }
...
async fn load_image<'a, 'b>(
    ...
    let is_srgb =
        !linear_textures.contains(&gltf_texture.index());
    let sampler_descriptor = texture_sampler(&gltf_texture);
    #[cfg(all(debug_assertions, feature = "dds"))]
    let name = gltf_texture.name().map_or(
        "Unknown GLTF Texture".to_string(),
        ToString::to_string,
    );
...
fn load_material(
    ...
    let material_label =
        material_label(material, is_scale_inverted);
...
fn warn_on_differing_texture_transforms(
    ...
    if has_differing_texture_transform {
        ...
        let texture_name = info
            ...
            .map(|n| {
                format!(
                    "its {texture_kind} texture \"{n}\""
                )
            })
            .unwrap_or_else(|| {
                format!(
                    "its unnamed {texture_kind} texture"
                )
            });
        let material_index = material
            .index()
...
fn load_node(
    ...
    let is_scale_inverted = world_transform
        .scale
        .is_negative_bitmask()
        .count_ones()
        & 1
        == 1;
    let mut node = world_builder
    ...
    if animation_context.is_none()
        && animation_roots.contains(&gltf_node.index())
    {
    ...
    #[cfg(feature = "bevy_animation")]
    if let Some(ref mut animation_context) =
        animation_context
    {
    ...
    if settings.load_cameras {
        if let Some(camera) = gltf_node.camera() {
            let projection = match camera.projection() {
                gltf::camera::Projection::Orthographic(
                    orthographic,
                ) => {
    ...
    node_index_to_entity_map
        .insert(gltf_node.index(), node.id());
    ...
    if !settings.load_meshes.is_empty() {
        if let (Some(mesh), Some(weights)) =
            (gltf_node.mesh(), morph_weights)
        {
            let primitive_label = mesh
                .primitives()
                .next()
                .map(|p| GltfAssetLabel::Primitive {
                    mesh: mesh.index(),
                    primitive: p.index(),
                });
            let first_mesh = primitive_label.map(|label| {
                load_context
                    .get_label_handle(label.to_string())
            });
            node.insert(MorphWeights::new(
                weights, first_mesh,
            )?);
        }
    }
    ...
    match texture.source().source() {
        ...
        Source::Uri { uri, .. } => {
            ...
            if let Ok(_data_uri) = DataUri::parse(uri) {
                load_context.get_label_handle(
                    GltfAssetLabel::Texture(
                        texture.index(),
                    )
                    .to_string(),
                )
...
fn texture_handle_from_info(
    ...
    let texture = document
        ...
        .expect(
            "Texture info references a nonexistent texture",
        );
...
fn inverse_bind_matrices_label(
    skin: &gltf::Skin,
) -> String {
...
fn texture_sampler(
    ...
    ImageSamplerDescriptor {
        address_mode_u: texture_address_mode(
            &gltf_sampler.wrap_s(),
        ),
        address_mode_v: texture_address_mode(
            &gltf_sampler.wrap_t(),
        ),
...
fn get_primitive_topology(
    mode: Mode,
) -> Result<PrimitiveTopology, GltfError> {
    match mode {
        ...
        mode => {
            Err(GltfError::UnsupportedPrimitive { mode })
        }
    }
}
...
impl<'a> GltfTreeIterator<'a> {
    ...
    fn try_new(
        ...
        while let Some(index) = empty_children.pop_front() {
            if let Some(skin) = unprocessed_nodes
                .get(&index)
                .unwrap()
                .0
                .skin()
            {
                ...
                let skin_has_dependencies =
                    skin.joints().any(|joint| {
                        unprocessed_nodes
                            .contains_key(&joint.index())
                    });

                if skin_has_dependencies
                    && unprocessed_nodes.len() != 1
                {
            ...
            let (node, children) =
                unprocessed_nodes.remove(&index).unwrap();
            assert!(children.is_empty());
            nodes.push(node);

            if let Some(parent_index) = parents[index] {
                let (_, parent_children) =
                    unprocessed_nodes
...
impl<'a> DataUri<'a> {
    fn parse(uri: &'a str) -> Result<DataUri<'a>, ()> {
        ...
        let (mime_type, data) =
            split_once(uri, ',').ok_or(())?;

        let (mime_type, base64) =
            match mime_type.strip_suffix(";base64") {
...
impl<'s> Iterator for PrimitiveMorphAttributesIter<'s> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let position =
            self.0 .0.as_mut().and_then(Iterator::next);
        let normal =
            self.0 .1.as_mut().and_then(Iterator::next);
        let tangent =
            self.0 .2.as_mut().and_then(Iterator::next);
        if position.is_none()
            && normal.is_none()
            && tangent.is_none()
        {
...
impl ClearcoatExtension {
    ...
    fn parse(
        ...
        Some(ClearcoatExtension {
            ...
            clearcoat_channel: clearcoat_channel
                .unwrap_or_default(),
            #[cfg(
                feature = "pbr_multi_layer_material_textures"
            )]
            ...
            clearcoat_normal_channel:
                clearcoat_normal_channel.unwrap_or_default(),
            #[cfg(
                feature = "pbr_multi_layer_material_textures"
            )]
            clearcoat_normal_texture,
    ...
    fn parse(
        ...
        Some(AnisotropyExtension {
            ...
            anisotropy_rotation: extension
                ...
                .and_then(Value::as_f64),
            #[cfg(feature = "pbr_anisotropy_texture")]
            anisotropy_channel: anisotropy_channel
                .unwrap_or_default(),
            #[cfg(feature = "pbr_anisotropy_texture")]
            anisotropy_texture,
        })
...
mod test {
    ...
    use bevy_render::mesh::{
        skinning::SkinnedMeshInverseBindposes, MeshPlugin,
    };
    ...
    fn run_app_until(
        app: &mut App,
        mut predicate: impl FnMut(&mut World) -> Option<()>,
    ) {
...
"#,
        ...
        let gltf_root_assets =
            app.world().resource::<Assets<Gltf>>();
        let gltf_node_assets =
            app.world().resource::<Assets<GltfNode>>();
        let gltf_root =
            gltf_root_assets.get(&handle).unwrap();
        assert!(
            gltf_root.nodes.len() == 1,
            "Single node"
        );
        assert!(
            gltf_root
                .named_nodes
                .contains_key("TestSingleNode"),
...
"#,
        ...
        let gltf_node_assets =
            app.world().resource::<Assets<GltfNode>>();
        let gltf_skin_assets =
            app.world().resource::<Assets<GltfSkin>>();
        ...
        let skin = gltf_skin_assets
            .get(&gltf_root.skins[0])
            .unwrap();
        assert_eq!(skin.joints.len(), 2);
        assert_eq!(skin.joints[0], gltf_root.nodes[1]);
        assert_eq!(skin.joints[1], gltf_root.nodes[2]);
        assert!(gltf_inverse_bind_matrices
            .contains(&skin.inverse_bind_matrices));

        let skinned_node = gltf_node_assets
            .get(&gltf_root.nodes[0])
            .unwrap();
        assert_eq!(skinned_node.name, "skinned");
        assert_eq!(skinned_node.children.len(), 2);
        assert_eq!(
            skinned_node.skin.as_ref(),
            Some(&gltf_root.skins[0])
        );
    }
}
